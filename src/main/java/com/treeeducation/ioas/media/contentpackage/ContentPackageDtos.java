...
public final class ContentPackageDtos {
    ...
    @Schema(description = "主题包自动计算目录路径")
    public record FolderPath(Long operatorId, String operatorName, Integer year, Integer month, Integer day, String topicName) {}
    ...
    public record Response(Long id, String packageNo, String topicName, Long operatorId, String operatorName,
                           FolderPath folderPath, String fullPath, String coverUrl, Integer scriptCount, Integer videoCount,
    ...
    public static Response of(ContentPackage p) {
        FolderPath folderPath = new FolderPath(p.getOperatorId(), p.getOperatorName(), p.getFolderYear(),
                p.getFolderMonth(), p.getFolderDay(), p.getTopicName());
        return new Response(p.getId(), p.getPackageNo(), p.getTopicName(), p.getOperatorId(), p.getOperatorName(),
                folderPath, p.getFullPath(), p.getCoverUrl(), p.getScriptCount(), p.getVideoCount(), p.getImageCount(),
