...
LEFT JOIN sys_user current_user ON current_user.id = l.assigned_to
...
SET l.assigned_to = target_user.id,
    ...
    l.assign_reason = CONCAT('历史官网线索修复：原分配账号[', COALESCE(current_user.username, '不存在账号'), ']不是有效顾问，已按意向区域改分配给顾问[', target_user.display_name, ']'),
...
WHERE l.source_type = 'official_website'
  AND (
      ...
      OR current_user.id IS NULL
      OR current_user.role_code <> 'CONSULTANT'
